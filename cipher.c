...
int encryptData(unsigned long m, unsigned long c, int ch,unsigned long x)
{
  int out=ch^(x%128); /* converts data into encrypted ascii*/
  if(m ==0 || c== 0) /*invalid lcg_m and lcg_c*/
    {
      printf("Error");
      return 1;
    }
...
int decryptData(unsigned long m, unsigned long c, int ch,unsigned long x)
{
  /*Converts into ascii*/
  int out=ch^(x%128);
  /*Base case for input error*/
  if(m==0 || c==0) 
    {
      printf("Error");
      return 1;
    } 
  /*if ascii is special character for decryption*/
  if(ch == '*')  
    {
      ch=getchar(); 
      if(ch == '*')
